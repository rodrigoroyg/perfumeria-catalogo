...
export default function AdminPanel() {
  ...
  async function cargarDivisas() {
    try {
      const { data, error } = await supabase.from('divisas').select('*');
      ...
      console.log('Usando cotización predeterminada por contingencia');
    }
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100 font-sans p-4">
        <form onSubmit={validarAcceso} className="bg-white p-8 rounded-xl shadow-md w-full max-w-sm space-y-4 border">
          <div className="text-center mb-4">
            <h2 className="text-2xl font-bold text-gray-800">Panel Admin Zafir</h2>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 font-sans text-gray-800">
      <header className="max-w-5xl mx-auto bg-white p-4 rounded-lg shadow mb-6 flex justify-between items-center">
        <h1 className="text-xl font-bold">Panel Admin - Perfumería Zafir</h1>
      ...
      <main className="max-w-5xl mx-auto bg-white p-6 rounded-lg shadow">
        {tab === 'ingreso' && (
          <form onSubmit={guardarProducto} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <h2 className="col-span-2 text-lg font-bold border-b pb-2">Alta de Producto / Factura</h2>
            ...
            <button type="submit" disabled={guardando} className="col-span-2 bg-green-600 text-white py-2 rounded font-bold hover:bg-green-700 disabled:bg-gray-400">
        ...
        {tab === 'divisas' && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">Al cambiar la tasa, los precios en PYG y BRL se recalcularán automáticamente.</p>
            <div className="flex gap-4 items-center">
              <span>1 USD =</span>
              <input type="number" value={monedas.PYG} onChange={e => setMonedas({...monedas, PYG: parseFloat(e.target.value) || 0})} className="p-2 border rounded w-32" />
              <span>PYG</span>
            </div>
            <div className="flex gap-4 items-center">
              <span>1 USD =</span>
              <input type="number" step="0.01" value={monedas.BRL} onChange={e => setMonedas({...monedas, BRL: parseFloat(e.target.value) || 0})} className="p-2 border rounded w-32" />
              <span>BRL</span>
            </div>
          </div>
        )}
